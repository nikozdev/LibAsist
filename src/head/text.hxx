...
public: /* typedef */
    ...
    using this_t = thstr_t_t<t_char_t>;
...
public: /* codetor */
    ...
    constexpr
    thstr_t_t(const data_t*data, size_t size, hash_t hash) noexcept
    : orig{ data }, size{ size }, hash{ hash } {}

    constexpr
    thstr_t_t(const data_t*data, size_t size) noexcept
    : this_t{ make_hstr(data, size) } {}

    template<size_t t_size>
    constexpr
    thstr_t_t(const data_t*(&data)[t_size]) noexcept
    : this_t{ make_hstr(data, t_size) } {}

    explicit constexpr thstr_t_t(const_wrap_t wrap) noexcept
...
using shstr_t = thstr_t_t<schar_t>;
...
scstr_t operator ""_scstr(const char* mdata, size_t msize)
{ return mdata; }
[[nodiscard]] constexpr inline
sdstr_t operator ""_sdstr(const char* mdata, size_t msize)
{ return sdstr_t(mdata, msize); }

/* deduction guide for a static character array */
template<typename t_char_t, size_t t_size>
thstr_t_t(const t_char_t(&data)[t_size]) -> thstr_t_t<t_char_t>;
/* deduction guide for a dynamic character array */
template<typename t_char_t>
thstr_t_t(const t_char_t*data, const size_t size) -> thstr_t_t<t_char_t>;

template<typename t_char_t>
[[nodiscard]] constexpr inline
bool operator==(const thstr_t_t<t_char_t>&lhs, const thstr_t_t<t_char_t>&rhs)
...
/* user defined literal for hashed string */
[[nodiscard]] constexpr inline
shstr_t operator ""_hstrs(const char*mdata, size_t msize)
